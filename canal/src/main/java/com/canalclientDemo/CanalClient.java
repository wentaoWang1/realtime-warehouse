...
import com.protobuf.CanalModel;
//import com.itheima.cana_demo.CanalModel;
...
/*
  Author：     Tao.W
  D a te：     2022/7/27
  Description:  canal为cs架构，server直接部署在服务器上，客户端需要编写代码
*/
...
public class CanalClient {
    ...
    public static void main(String[] args) {
        //1：创建连接
        //CanalConnector canalConnector = CanalConnectors.newSingleConnector(new InetSocketAddress("192.168.44.4", 11111), "example", "", ""); //目前没有给example设置密码

        CanalConnector canalConnector = CanalConnectors.newClusterConnector("linux01:2181","example","","");
        ...
        try {
            ...
            while (isRunning) {
                ...
                if (size == 0 || size == -1) {
                    ...
                    //方式三：使用protobuf序列化 在写入到kafka中
                    /*
                        将消费到的binlog日志转换成字符串，将字符串写入到kafka中
                        因为存储到kafka的数据是json格式的话，json形式的缺点，虽然结构很公正，但很冗余,需要占用太多的存储资源和网络资源，
                    所以引进protobuf.将数据序列化后转为字节码发送到kafka
                    */
